<template>
  ...
  <header-nav></header-nav>
      ...
      <div class="item-box">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li @click="TableImg(index)" :class="{'on':index==ImageIndex}" :key="index" v-for="(img,index) in itemsInfo.ali_images"><img :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
            <div class="thumb">
              <ul>
                <li :class="{'on':index==ImageIndex}" :key="index" v-for="(img,index) in itemsInfo.ali_images"><img :src="img+'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
          </div>
        </div>
            ...
            <div class="sku-dynamic-params clear">
              ...
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  <span class="down "  :class="{  'down-disabled': this.count<=1 }" @click="subCount">-</span>
                  <span class="num" >{{count}}</span>
                  <span class="up"  :class="{  'up-disabled': this.count>=itemsInfo.limit_num }"  @click="addCount" >+</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <prompt v-if="this.$store.state.maxOff"></prompt>
  </div>
</template>
    ...
    export default {
      ...
      computed:{
        ...
        methods: {
          ...
          addCarPanelHandle(){
            let ItemData = [this.itemsInfo,this.count]
