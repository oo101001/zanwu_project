<template>
  <div class="item">
    <div>
        ...
        <ul class="colors-list">
          <li :key="index" v-for="(sku,index) in item.sku_info">
            <a href="javascript:;" @click="tableData(index)" :data-index="index" :title="sku.spec_json.show_name"
              :class="{'active':index==itemIndex}"><img :src="'http://img01.smartisanos.cn/'+ sku.spec_json.image +'/20X20.jpg'"></a>
          </li>
        </ul>
      </div> -->
      <div class="item-btns clearfix">
        <span
          v-if="getUserName == null && getPassWord == null"
          class="item-gray-btn"
        >
          <router-link :to="{ path: '/Login' }">查看详情</router-link>
        </span>
        <span v-else class="item-gray-btn">
          <router-link
            :to="{
              path: '/item',
              query: { ItemId: `${item.sku_info[itemIndex].sku_id}` }
            }"
            >查看详情</router-link
          >
        </span>
      ...
      <div v-if="getUserName == null && getPassWord == null" class="item-cover">
        <router-link :to="{ path: '/Login' }"></router-link>
      </div>
      <div v-else class="item-cover">
        <router-link
          :to="{
            path: '/item',
            query: { ItemId: `${item.sku_info[itemIndex].sku_id}` }
